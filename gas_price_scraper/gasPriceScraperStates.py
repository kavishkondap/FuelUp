...
countyLinks = []
countyName = []
index = pd.read_excel ('IndexStates.xlsx')
index = index ['Index']
index = index[0]
index = (int)(index)
...
currData = np.array (pd.read_excel ('FinalDataStates.xlsx'))
print (currData)
currDataArrAddress = []
currDataArrPrices = []
...
df.to_excel (r'C:\Users\kavis\Desktop\KAVISH\General Stuff\Gas Prices Project\FinalDataStates.xlsx', index = False, header=False)
indexArr = {"Index":(index+30)}

dfIndex = pd.DataFrame (indexArr, columns=['Index'], index={0})
dfIndex.to_excel (r'C:\Users\kavis\Desktop\KAVISH\General Stuff\Gas Prices Project\IndexStates.xlsx', index = False, header=True)
